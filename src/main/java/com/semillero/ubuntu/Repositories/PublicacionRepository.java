...
public interface PublicacionRepository extends JpaRepository <Publicacion, Long> {
    ...
    @Query(value = "SELECT p.* FROM publicacion p INNER JOIN usuario u ON p.id_usuario = u.id WHERE u.email = :email ORDER BY p.fecha_creacion DESC LIMIT 10", nativeQuery = true)
